...
class DataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        if self.data_augmentation:
            X = np.array([smiles_to_seq(self.sme.randomize_smiles(s), self.seq_length) for s in self.X[indexes]]) # Implementar
        else:
            X = np.array([smiles_to_seq(s, self.seq_length) for s in self.X[indexes]]) # Implementar
        y = self.y[indexes] # Implementar
        return X, y
    ...
    def on_epoch_end(self):
        ...
        if self.shuffle == True:
            np.random.shuffle(self.indexes) # Implementar